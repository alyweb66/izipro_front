...
import { userDataStore, userConversation } from '../../../store/UserData';
...
function MyRequest({
  ...
  const [requestByDate, setRequestByDate] = useState<RequestProps[] | null>(
    null
  );
  ...
  const [isAnswerOpen, setIsAnswerOpen] = useState<boolean>(
    window.innerWidth > 1000 ? true : false
  );
  const [isMessageOpen, setIsMessageOpen] = useState<boolean>(
    window.innerWidth > 1000 ? true : false
  );
  ...
  ] = useMutation(DELETE_REQUEST_MUTATION);
  const [
    message,
    { loading: messageMutationLoading, error: createMessageError },
  ...
  const { loading: requestLoading, fetchMore } = useQueryUserRequests(
    id,
    0,
    limit,
    isSkipRequest
  );
  const { loading: conversationLoading, usersConversationData } =
    useQueryUsersConversation(
      newUserId.length !== 0 ? newUserId : userIds,
      0,
      0
    );
  const { loading: messageLoading, messageData } =
    useQueryMyMessagesByConversation(fetchConvIdState, 0, 100, isSkipMessage);
  ...
  const handleDeleteRequest = (requestId?: number) => {
    deleteRequest({
      ...
      if (response.data.deleteRequest) {
        ...
        setMyRequestsStore(
          myRequestsStore.filter((request) => request.id !== requestId)
        );
        ...
        const updatedSubscription = Array.isArray(subscription?.subscriber_id)
          ? subscription?.subscriber_id.filter((id) => id !== requestId)
          : [];
        ...
        const conversationSubscription = subscriptionStore.find(
          (subscription) => subscription.subscriber === 'conversation'
        );
        // get array of conversation ids of request id
        const conversationIds = myRequestsStore.find(
          (request) => request.id === requestId
        )?.conversation;

        // remove the conversation id from the subscription
        const updatedConversationSubscription = Array.isArray(
          conversationSubscription?.subscriber_id
        )
          ? conversationSubscription?.subscriber_id.filter(
              (id) => !conversationIds?.some((conv) => conv.id === id)
            )
          : [];
        // update the subscription
        const newConversationSubscription = {
  ...
  const handleMessageConversation = (
    ...
    const conversation = selectedRequest?.conversation?.find(
      (conversation) =>
    ...
    if (
      ...
      !isMessageConvIdFetched.some((convId) => convId === conversationId)
    ) {
    ...
    if (
      conversationId &&
      notViewedConversationStore?.some((id) => id === conversationId)
    ) {
      setNotViewedConversationStore(
        notViewedConversationStore.filter((id) => id !== conversationId)
      );
  ...
  const addRequest = () => {
    if (fetchMore && !isFetchingMore) {
      ...
      fetchMore({
        ...
        (fetchMoreResult: { data: { user: { requests: RequestProps[] } } }) => {
          // remove request who is already in the store
          const requestsIdsStore = myRequestsStore.map((request) => request.id);

          const newRequests = fetchMoreResult.data.user.requests.filter(
            (request: RequestProps) => !requestsIdsStore.includes(request.id)
          );
          ...
          if (newRequests.length > 0) {
            ...
            offsetRef.current =
              offsetRef.current + fetchMoreResult.data.user.requests.length;
          }
    ...
    if (myRequestsStore) {
      ...
      const sortedRequests = [...myRequestsStore].sort((requestA, requestB) => {
        const dateA = requestA.conversation?.length
          ? Math.max(
              ...requestA.conversation.map((conv) =>
                isNaN(Date.parse(conv.updated_at))
                  ? Number(conv.updated_at)
                  : new Date(conv.updated_at).getTime()
              )
            )
          : isNaN(Date.parse(requestA.created_at))
            ? Number(requestA.created_at)
            : new Date(requestA.created_at).getTime();

        const dateB = requestB.conversation?.length
          ? Math.max(
              ...requestB.conversation.map((conv) =>
                isNaN(Date.parse(conv.updated_at))
                  ? Number(conv.updated_at)
                  : new Date(conv.updated_at).getTime()
              )
            )
          : isNaN(Date.parse(requestB.created_at))
            ? Number(requestB.created_at)
            : new Date(requestB.created_at).getTime();
  ...
  useEffect(() => {
    if (userConvStore && selectedRequest && selectedRequest.conversation) {
      ...
      const filteredSortedUsers = sortedUsers.filter(
        (user): user is UserDataProps => user !== undefined
      );
      // Convert filteredSortedUsers to a Set to remove duplicates, then convert it back to an array
      const uniqueUsers = Array.from(
        new Set(filteredSortedUsers.map((user) => JSON.stringify(user)))
    ...
    if (
      !selectedRequest ||
      selectedRequest.id !== selectedRequestRef.current?.id
    ) {
  ...
  useEffect(() => {
    if (messageData) {
      ...
      setIsMessageConvIdFetched([
        ...isMessageConvIdFetched,
        conversationIdState,
      ]);
  ...
  useEffect(() => {
    if (usersConversationData) {
      ...
      if (userConvStore.length === 0) {
        ...
        usersConversationData.users.every((user1: UserDataProps) =>
    ...
    <div className="my-request">
      {isListOpen && (
        <div className="my-request__container">
          {requestByDate && isListOpen && requestByDate?.length > 0 && (
            <button
              className="my-request__container__deploy"
              onClick={() => setShowAllContent(!showAllContent)}
            >
              {requestByDate &&
                requestByDate?.length > 0 &&
                (showAllContent
                  ? 'Réduire les demandes'
                  : 'Déployer les demandes')}
            </button>
          )}
          <div
          ...
          >
            {!requestByDate ? (
              <p className="my-request__container__list no-req">
        ...
        {isAnswerOpen && (
          ...
          >
            ...
            <div className="my-request__answer-list__header">
              ...
              />
              {selectedRequest && (
                <h2 className="my-request__answer-list__header title">
                  {selectedRequest?.title}
                </h2>
              )}
            </div>

            {userConvState?.length === 0 ? (
              <p className="my-request__answer-list no-conv">
              ...
              <div className="my-request__answer-list__container">
                <AnimatePresence>
                  {isAnswerOpen &&
                    userConvState &&
                    userConvState?.map((user: UserDataProps, index) => (
                      <motion.div
                        ...
                        className={`my-request__answer-list__user 
							...
							${
                selectedRequest?.conversation.some(
                  (conv) =>
                    (notViewedConversationStore?.some((id) => id === conv.id) &&
                      conv.user_1 === user.id) ||
                    conv.user_2 === user.id
                )
                  ? 'not-viewed'
                  : ''
                      ...
                      >
                        <div className="my-request__answer-list__user__header">
                          ...
                          {user.denomination ? (
                            <p className="my-request__answer-list__user__header denomination">
                              {user.denomination}
                            </p>
                          ) : (
                            <p className="my-request__answer-list__user__header name">
                              {user.first_name} {user.last_name}
                            </p>
                          )}
                          {user.deleted_at && (
                            <p
                              className="my-request__answer-list__user__header deleted"
      ...
      <AnimatePresence>
        {isMessageOpen && (
          <motion.div
            ...
            transition={{ duration: 0.1, type: 'tween' }}
          >
            {(messageLoading || messageMutationLoading || isSendingMessage) && (
              <Spinner />
            )}
            <HeaderMessage
