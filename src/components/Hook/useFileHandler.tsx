...
export function useFileHandler() {
  ...
  const [urlFile, setUrlFile] = useState<{ file: { name: string; type: string }; url: string }[]>([]);
  ...
  const handleFileChange = (
    ...
    const validFiles = Array.from(files!).filter((file) => {
      ...
      if (
        ...
        if (
          ...
          file.size > maxPdfFileSize
        ) {
          setFileError(
            `Fichier ${file.name} est trop grand, veuillez choisir un PDF de moins de 1Mo.`
          );
          setTimeout(() => {
            setFileError('');
          }, 15000);
        ...
        if (
          ...
          setFileError(
            `Fichier ${file.name} est trop grand, veuillez choisir une image de moins de 15Mo.`
          );
          setTimeout(() => {
            setFileError('');
          }, 15000);
        ...
        setFileError(
          `Fichier ${file.name} n'est pas un fichier valide, fichiers acceptés .jpg, .jpeg, .png ou .pdf.`
        );
        setTimeout(() => {
          setFileError('');
        }, 15000);
    ...
    if (validFiles) {
      ...
      const fileObjects = urls.map((url) => ({
        file: new File([url.file], url.fileName, { type: url.file.type }),
        url: url.url,
      }));
