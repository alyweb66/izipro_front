...
type MessageFormProps = {
  ...
  urlFile: { file: { name: string; type: string }; url: string }[];
        ...
        <div className="message-form__preview">
          {urlFile.map((file, index) => (
            <div className="message-form__preview__container" key={index}>
              <img
                className={`message-form__preview__container__image ${file.file.type === 'application/pdf' ? 'pdf' : ''}`}
                src={file.file.type === 'application/pdf' ? pdfLogo : file.url}
                alt={`Preview ${index}`}
              />
      ...
      <label className="message-form__label">
        ...
        <TextareaAutosize
          ...
          onChange={(event: React.ChangeEvent<HTMLTextAreaElement>) =>
            setMessageValue(DOMPurify.sanitize(event.target.value))
          }
          placeholder="Tapez votre message ici..."
          aria-label="Tapez votre message ici"
          maxLength={1000}
          readOnly={
            (selectedItem && selectedItem?.id > 0 ? false : true) ||
            (selectedItem && 'deleted_at' in selectedItem)
              ? selectedItem?.deleted_at !== null
              : false
          }
