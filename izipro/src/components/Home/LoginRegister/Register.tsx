...
function Register({ setLoginVisibility, loginVisibility }: RegisterProps) {
  ...
  const [
    createProUser,
    { loading: proUserLoading /* , error: proUserError */ },
  ] = useMutation(REGISTER_PRO_USER_MUTATION);
  ...
  const handleProRegister = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (proPassword && !validator.isStrongPassword(proPassword)) {
      setIsProError(
        'Mot de passe faible, doit contenir au moins 8 caractères, une majuscule, une minuscule, un chiffre et un caractère spécial'
      );
      setTimeout(() => {
        setIsProError('');
      }, 15000);
    ...
    if (!payload && altchaStatus === 'error') {
      setIsProError('Erreur lors de la vérification de sécurité');
      setTimeout(() => {
        setIsProError('');
      }, 15000);
      return;
  ...
  return (
    <div className="register-container">
      <p className="register-container title">
        <span
          onClick={() => {
              toggleRegisterVisibility();
              window.innerWidth < 480 && setLoginVisibility(!loginVisibility);
          }}
        >
          {' '}
          Créer un compte{' '}
      ...
      <AnimatePresence>
        {isRegisterVisible && (
          ...
          >
            <form
              className="register-container__form__form"
              onSubmit={(event) => handleRegister(event)}
            >
              {userLoading && <Spinner />}
              <p className="register-container__form__form category">
                Particulier
              </p>
              <input
                type="email"
                  ...
                  {userCreated && (
                    <Fade in={!!userCreated} timeout={300}>
                      <Alert variant="filled" severity="success">
                        Utilisateur créé avec succès, un email de validation
                        vous a été envoyé (pas de mail? vérifiez vos spams)
                      </Alert>
                    </Fade>
                  )}
                </Stack>
              ...
              <button
                type="submit"
                className="register-container__form__form button"
              >
                Enregistrer
              </button>
            ...
            <form
              className="register-container__form__form"
              onSubmit={(event) => handleProRegister(event)}
            >
              ...
              <input
                ...
                placeholder="Adresse e-mail"
                onChange={(event: React.ChangeEvent<HTMLInputElement>) =>
                  setProEmail(event.target.value.toLowerCase())
                }
                aria-label="Adresse e-mail"
                maxLength={50}
                required
              ...
              <div className="show-password">
                <input
                  ...
                  onChange={(event: React.ChangeEvent<HTMLInputElement>) =>
                    setProConfirmPassword(event.target.value)
                  }
                ...
                <span
                  className="toggle-password-icon"
                  onClick={() =>
                    setShowProConfirmPassword(!showProConfirmPassword)
                  }
                >
                  {showProConfirmPassword ? (
                    <MdOutlineVisibilityOff />
                  ) : (
                    <MdOutlineVisibility />
                  )}
              ...
              <input
                ...
                onChange={(event: React.ChangeEvent<HTMLInputElement>) =>
                  setSiret(event.target.value.replace(/\s+/g, ''))
                }
                aria-label="Siret"
                maxLength={14}
                required
              ...
              <button
                type="submit"
                className="register-container__form__form button pro"
              >
                Enregistrer
              </button>
			  <Altcha onSubmit={true}/>
