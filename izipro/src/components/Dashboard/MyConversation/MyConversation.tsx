...
import { DELETE_NOT_VIEWED_CONVERSATION_MUTATION } from '../../GraphQL/ConversationMutation'; // Assuming these mutations are from ConversationMutation
...
function MyConversation({
  ...
  const [
    hideRequest,
    { loading: hideRequestLoading, error: hideRequestError },
  ] = useMutation(USER_HAS_HIDDEN_CLIENT_REQUEST_MUTATION);
  const [
  ...
  //query
  const { loading: convLoading, fetchMore } = useQueryUserConversations(
    0,
    limit,
    true
  ) as unknown as useQueryUserConversationsProps;
  const { loading: messageLoading, messageData } =
    useQueryMessagesByConversation(fetchConvIdState, 0, 100, isSkipMessage);
      ...
      if (messageValue.trim() !== '' || sendFile.length > 0) {
        ...
        message({
          ...
          if (response.data.createMessage?.id > 0) {
            ...
            setUploadFileError(
              'Demande supprimée, actualisez la page pour verifier'
            );
            setTimeout(() => {
              setUploadFileError('');
            }, 10000);
          ...
          const newRequest = clientRequestsStore.filter(
            (clientRequest) => clientRequest.id !== request.id
          );
    ...
    if (fetchMore) {
      fetchMore({
        ...
        (fetchMoreResult: {
          ...
          const newRequests = request.filter((request: RequestProps) =>
  ...
  const handleHideRequest = (requestId?: number) => {
    ...
    hideRequest({
      ...
      const requestConversation = requestsConversationStore.find(
        (request) => request.id === requestId
      );
      const conversationId = requestConversation?.conversation.find(
        (conversation) =>
      ...
      if (response.data.createHiddenClientRequest) {
        ...
        const updatedStore = requestsConversationStore.filter(
          (request) => request.id !== requestId
        );
        ...
        const subscription = subscriptionStore.find(
          (subscription: SubscriptionProps) =>
            subscription.subscriber === 'clientConversation'
        );
        const newSubscriptionIds = Array.isArray(subscription?.subscriber_id)
          ? subscription.subscriber_id.filter(
              (id: number) => id !== conversationId
            )
          : [];

        // update the subscription store
        subscriptionDataStore.setState(
          (prevState: SubscriptionStore): Partial<SubscriptionStore> => ({
            ...prevState,
            subscription: prevState.subscription.map((subscription) =>
              subscription.subscriber === 'clientConversation'
                ? { ...subscription, subscriber_id: newSubscriptionIds }
                : subscription
            ),
          })
        );
  ...
  const handleViewedMessage = (convId: number) => {
    if (
      selectedRequest &&
      notViewedConversationStore?.some((id) => id === convId)
    ) {
      deleteNotViewedConversation({
        variables: {
          input: {
        ...
        // remove the conversation id from the notViewedConversationStore
        setNotViewedConversationStore(
          notViewedConversationStore.filter((id) => id !== convId)
        );
  ...
  useEffect(() => {
    if (selectedRequest && selectedRequest.id > 0) {
      const conversationId = selectedRequest.conversation?.find(
        (conversation) =>
          conversation.user_1 === id || conversation.user_2 === id
      );
      ...
      if (
        conversationId?.id !== 0 &&
        !isMessageConvIdFetched.some((convId) => convId === conversationId?.id)
      ) {
      ...
      if (viewedMyConversationState.length > 0) {
        const convId = selectedRequest?.conversation?.find(
          (conv) => conv.user_1 === id || conv.user_2 === id
        )?.id;
        if (convId) {
    ...
    if (requestsConversationStore.length > 0) {
      const sortedRequests = [...requestsConversationStore].sort((a, b) => {
        ...
        const dateA = a.conversation.some((c) => c.updated_at)
          ? Math.max(
              ...a.conversation.map((c) => new Date(c.updated_at).getTime())
            )
          : 0;

        const dateB = b.conversation.some((c) => c.updated_at)
          ? Math.max(
              ...b.conversation.map((c) => new Date(c.updated_at).getTime())
            )
          : 0;
  ...
  useEffect(() => {
    return () => {
      ...
      if (
        !isNewConversation ||
        (isNewConversation !== 'true' && request.id > 0)
      ) {
        // remove request.id in requestsConversationStore

        const removedRequest = requestConversationStore
          .getState()
          .requests?.filter(
            (requestConv: RequestProps) => request.id !== requestConv.id
          );
      ...
      {isListOpen && (
        <div className="my-conversation__container">
          {requestByDate && isListOpen && requestByDate?.length > 0 && (
            <button
              className="my-conversation__container__deploy"
          ...
          <div
            id="scrollableList"
            className={`my-conversation__container__list ${isListOpen ? 'open' : ''}`}
          >
            ...
            {isHasMore && requestByDate && requestByDate?.length > 0 ? (
              <FetchButton addRequest={addRequest} />
            ) : (
              <p className="my-conversation__container__list no-req">
                Fin des résultats
              </p>
            )}
          </div>
      ...
      <AnimatePresence>
        {isMessageOpen && (
          <motion.div
            ...
            transition={{ duration: 0.1, type: 'tween' }}
          >
            {(messageLoading || messageMutLoading || isSendingMessage) && (
              <Spinner />
            )}
